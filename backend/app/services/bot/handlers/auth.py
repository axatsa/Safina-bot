...
from ..utils import get_user_templates
        ...
        if user:
            ...
            if user.position == "ceo":
                ...
                await message.answer(
                    ...
                    reply_markup=get_main_kb(is_ceo=False, templates=templates)
                )
            return
...
async def process_login(message: types.Message, state: FSMContext):
    ...
    with next(database.get_db()) as db:
        ...
        if user.position == "ceo":
            ...
            templates = get_user_templates(user)
            await message.answer(f"✅ Успешно, {user.first_name}!", reply_markup=get_main_kb(is_ceo=False, templates=templates))
