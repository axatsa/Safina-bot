...
def get_user_templates(user) -> list[str]:
    """Собирает уникальный список шаблонов из всех проектов + личные."""
    res = set(user.templates or [])
    for p in user.projects:
        if p.templates:
            res.update(p.templates)
        
    # Сортируем для стабильности порядка кнопок
